...
router.post('/enterprise/seller/', async (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_KEY, function (err, decoded) {
    if (err) return res.json({ success: false, err })
      ...
      var query = 'Select Member.enterpriseId, Seller.sellerId, Seller.name, Seller.minPrice, Seller.imgURL from Member join Contract on Contract.enterpriseId = Member.enterpriseId join Seller on Contract.sellerId = Seller.sellerId where customerId = :customerId and Contract.approval = 1 and Member.approval = 1;'
      var values = { // query에서 :customerId -> decode.customerId로 변환
        customerId: decoded.customerId
      }
      db.sequelize.query(query, { replacements: values }).spread(function (results, subdata) { // results 뭐하는건지 모르겠음
